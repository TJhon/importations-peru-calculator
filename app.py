...
if st.button("Registrar", use_container_width=True, type="primary"):
    ...
    taxes_result = [add_data(result.model_dump(), ClientPITaxes) for result in results]
    taxes_result = [add_data(result.model_dump(), TaxesValues) for result in results]
    result = results[0]
    logistic = add_data(result.model_dump(), LogisticCost)

    st.success("Registro Completo")

    # # taxes_result = [ClientPITaxes(**result.model_dump()) for result in results]
    # taxes_values = [TaxesValues(**result.model_dump()) for result in results]
    # st.write(taxes_result)
